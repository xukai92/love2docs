...
class Love2DWikiScraper:
    ...
    def save_pages_individually(self, pages: List[Dict], output_folder: str):
        """Save each page as an individual text file."""
        ...
        print(f"Saved {saved_count} pages to {output_path}/ directory")
...
@click.option('--save-each', is_flag=True, help='Save each page as individual file in output folder')
def main(concurrency: int, output: str, prefix: str, save_each: bool):
    ...
    # Save pages based on the save_each flag
    if save_each:
        # Remove file extension from output to use as folder name
        output_folder = output.rsplit('.', 1)[0] if '.' in output else output
        scraper.save_pages_individually(love_api_pages, output_folder)
    else:
